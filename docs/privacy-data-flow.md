...
Import extraction is browser-local as well. A selected file is validated and processed in memory only; importing does not call Supabase, Groq, analytics, a Netlify Function, or a third-party parser. The raw file, complete extracted text, and source evidence are kept only for the active review session and are discarded on cancel or after the reviewed structured resume is created. They are never added to ATS history, error reporting, analytics, or console logs. Import errors identify the file condition without returning raw document contents.
